...
export const getProposol = async (email) => {
  const res = await fetch(`${baseUrl}/api/proposals?email=${email}`);
  return res.json() || [];
};
export const getProposolPending = async (email) => {
  const res = await fetch(`${baseUrl}/api/proposals/pending?email=${email}`);
  return res.json() || [];
};
