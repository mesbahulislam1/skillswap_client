  ...
  return (
    <div className="w-[90%] mx-auto p-8 bg-white min-h-screen font-sans">
      ...
      <header className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 tracking-tight">My Proposals</h1>
          ...
          >
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap items-center gap-3 pt-2 text-sm text-gray-400">
                <span className="font-bold text-gray-900">${proposal?.proposedBudget}</span>
                <span>{proposal.duration}</span>
                <span>{proposal?.createdAt}</span>
                <span className='font-medium'>Task budget: ${proposal.taskBudget}</span>
              </div>
            </div>

            {/* Right Side Status Badge */}
            <div>
              <span className="capitalize border-[#FE9C06] text-[#FE9C06] bg-[#FE9C06]/9 border text-xs font-medium px-3 py-1 rounded-full whitespace-nowrap">
                {proposal?.status}
              </span>
            </div>
          </div>
